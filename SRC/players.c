...
void goForward(player * p){
    ...
    if ((p->x + dx < X_MIN || p->x + dx > X_MAX || !isFree(floor(p->x + dx + ((dx > 0)?1:-1) * X_HITBOX), floor(p->z))) && !kTab[NOCLIP_BIT]){
	dx = 0;
    }
    if ((p->z + dz < Z_MIN || p->z + dz > Z_MAX || !isFree(floor(p->x), floor(p->z + dz + ((dz > 0)?1:-1) * Z_HITBOX))) && !kTab[NOCLIP_BIT]){
	dz = 0;
    }
    p->x += dx;
    p->y += 0;
